...
class GasConcentrationMLP(nn.Module):
    def __init__(self, input_features=3600, num_gases=11, hidden_layers=[512, 256, 128]):
        super().__init__()
        layers = []
    ...
    def forward(self, x):
        # x: (B, 1, 3600) → flatten
        x = x.view(x.size(0), -1)  # (B, 3600)
        return self.network(x)     # (B, 10)
...
def evaluate_model(model, loader, criterion, device, weights_for_loss=None):
    model.eval()
    running_loss = 0.0
    with torch.no_grad():
        for x, y in loader:
            x, y = x.to(device), y.to(device)
            pred = model(x)
            if weights_for_loss is not None: # For weighted U-Net loss
                loss = ((pred - y)**2 * weights_for_loss).mean()
            else: # For standard MSE (e.g. MLP)
...
def eval_metrics(model, loader, device, y_scaler):
    ...
    return r2.numpy(), nrmse.numpy()

# Gradient-based weights (train Jacobians × val MSE)
def compute_blend_weights(
        unet, mlp,
        train_loader_no_shuffle, x_scaler,
        mse_val_gas: torch.Tensor,
        device: torch.device,
        target_gas_indices: list[int],
    ...
    for noisy, _ in train_loader_no_shuffle:
        ...
        # 2) apply scaler
        np_scaled = x_scaler.transform(np_in)       # shape stays [B,3600]
        # 3) back to torch on device, restore [B,1,3600]
        ...
        for gas_idx in target_gas_indices:
            ...
            weighted_ig = attributions_for_gas.abs().sum(dim=0).squeeze()
            accum += weighted_ig
...
def main(a):
    ...
    for it in range(1,a.iters+1):
        ...
        for ep in range(1,a.epochs_m+1):
            ...
            r2, _ = eval_metrics(mlp,va_reg,device,y_scaler); r2m=float(r2.mean())
        ...
        # --- D) test metrics
        r2_t,nrmse_t = eval_metrics(mlp,te_reg,device,y_scaler)
        print("\nPerformance on TEST set (iteration {}):".format(it))
        for name, r2_g, nrmse_g in zip(labels, r2_t, nrmse_t):
            print(f"  {name:15s}  R²={r2_g:6.3f}   NRMSE={nrmse_g:7.4f}")
        json.dump({"R2":r2_t.tolist(),"NRMSE":nrmse_t.tolist()},
                  open(outdir/f"metrics_iter{it}.json","w"), indent=2)

        # --- E) blend weights  (Jacobs on TRAIN  ×  MSE on VAL)
        # MSE per gas on val
        mlp.eval(); pv, tv = [],[]
        with torch.no_grad():
            for x,y in va_reg:
        ...
        for idx, gas_name in enumerate(labels):
            print(f"  → computing & plotting weights for {gas_name} (index {idx})")
            # compute only for this single gas
            w_gas = compute_blend_weights(
                unet, mlp,
                tr_base_noshuff, x_scaler,
                mse_val, device,
                target_gas_indices=[idx]
            ).detach()
    
            # save raw numpy and plot
            np.save(outdir / f"w_{gas_name.replace(' ', '_')}.npy",
                    w_gas.cpu().numpy())
            plot_weights(
                w_gas,
                outdir / f"w_{gas_name.replace(' ', '_')}.png"
            )
        return
...
if __name__=="__main__":
    ...
    a.mlp = "/vol/csedu-nobackup/project/jlandsman/trained_models/mlp_pretrained/mlp_weights.pth"
    a.results_root = "./weights_plots"
    a.bs = 128
    a.lr_u = 1e-4
    a.lr_m = 2e-4
    a.epochs_u = 15
