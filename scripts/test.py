...
def eval_metrics(model, loader, device, y_scaler):
    ...
    return r2.numpy(), nrmse.numpy(), nrmse.numpy()
...
def main(a):
    ...
    print("With smoothing target baseline, unet 51 poly 2, focus on Nitrous Oxide, Acetone, Ethanol")

    labels = ['Water', 'Carbon Dioxide', 'Methane', 'Ammonia', 'Methanol', 'Nitrous oxide', 'Ethylene', 'Acetone', 'Isoprene', 'Acetaldehyde', 'Ethanol']
    focus_gases = ['Nitrous Oxide', 'Acetone', 'Ethanol']
    ...
    # Smoothing target baselines
    sg_window = 51
    sg_polyorder = 2
    Btr_smooth = savgol_filter(Btr, sg_window, sg_polyorder, axis=1)
    Bva_smooth = savgol_filter(Bva, sg_window, sg_polyorder, axis=1)
    Bte_smooth = savgol_filter(Bte, sg_window, sg_polyorder, axis=1)
    ...
    # loaders
    tr_base   = make_loader(Xtr[:,None,:], Btr_smooth[:,None,:], a.bs, True)
    tr_base_noshuff = make_loader(Xtr[:,None,:], Btr_smooth[:,None,:], a.bs, False)
    va_base   = make_loader(Xva[:,None,:], Bva_smooth[:,None,:], a.bs, False)
    te_base   = make_loader(Xte[:,None,:], Bte_smooth[:,None,:], a.bs, False)
...
if __name__=="__main__":
    ...
    a.unet = "/vol/csedu-nobackup/project/jlandsman/jobs/full_proc_results/0907_1307/unet_iter10.pth"
    a.mlp = "/vol/csedu-nobackup/project/jlandsman/jobs/full_proc_results/0907_1307/mlp_iter10.pth"
