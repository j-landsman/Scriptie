...
def compute_blend_weights(unet, mlp,
                           train_loader_no_shuffle, x_scaler,
                           mse_val_gas: torch.Tensor,   # (11,)
                           device: torch.device,
    ...
    mse_bc = mse_val_gas[target_gas_indices].cpu().view(1,-1,1)    
    ...
    for noisy,_ in train_loader_no_shuffle:
        ...
        J_full = vmap(jac_fn)(cleaned_norm).abs()                  # (B,11,3600)
        J = J_full[:, target_gas_indices, :]
        J = J/(J.sum(-1,keepdim=True)+1e-12)                 # Normalize per gas
        weighted_sum = (J*mse_bc).sum((0,1))
        accum += weighted_sum

        del J_full, J, cleaned, cleaned_cpu, baseline_pred
    weights = accum / total_samples
    weights = weights / (weights.max() + 1e-12)              # Scale between 0 and 1  
...
def main(a):
    ...
    target_gas_names = ['Water', 'Carbon Dioxide', 'Methane', 'Ammonia', 'Methanol', 'Ethylene', 'Acetone', 'Isoprene', 'Acetaldehyde', 'Ethanol']
    target_gas_indices = [labels.index(name) for name in target_gas_names]
    ...
    # scalers
    # y_scaler = joblib.load(a.y_scaler) if a.y_scaler else MinMaxScaler().fit(Ytr)
    ...
    for it in range(1,a.iters+1):
        ...
        weights = compute_blend_weights(unet,mlp,tr_base,x_scaler,mse_val,device, target_gas_indices).detach()
