...
def main():
    ...
    OUTPUT_DIR = "mlp_pretrained/newscale"
    ...
    X_train1 = tr1["inputs_lntrans_1"].astype(np.float32)
    Y_train1 = np.delete(tr1["c"], 7, axis=1).astype(np.float32)

    tr2 = np.load(TRAIN_FILE2)
    X_train2 = tr2["inputs_lntrans_1"].astype(np.float32)
    ...
    X_val1 = va1["inputs_lntrans_1"].astype(np.float32)
    Y_val1 = np.delete(va1["c"], 7, axis=1).astype(np.float32)

    va2 = np.load(VAL_FILE2)
    X_val2 = va2["inputs_lntrans_1"].astype(np.float32)
    ...
    X_test = te["inputs_lntrans_1"].astype(np.float32)
    Y_test = np.delete(te["c"], 7, axis=1).astype(np.float32)

    # Scale inputs and targets
    CaliX = np.array([[0,5],[0,6],[0,40],[0,2000],[0,2000],[0,4000],[0,200],[0,4000],[0,2000],[0,30],[0,5000]])
    mins = CaliX[:, 0]
    maxs = CaliX[:, 1]
    Y_train_scaled = (Y_train - mins) / maxs
    Y_val_scaled = (Y_val - mins) / maxs
    Y_test_scaled = (Y_test - mins) / maxs
    ...
    model = GasConcentrationMLP().to(device)
    ...
    y_val_pred = y_val_pred_scaled * maxs
    ...
    y_test_pred = y_test_pred_scaled * maxs
